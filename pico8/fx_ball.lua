function fx_ball()
  pal()
  local gpal = {1,5,6,15,10}
  local sn = 15
  local gmode = 2
  local balls = {
    {o = {0,0,0}, a = {1, 0.7, 0.1}, g = {}},
    {o = {0,0,0}, a = {0.94, 0.33, 0.2}, g = {}},
  }
  ...
  local function pj(u, v, r, o)
    ...
    local x = cos(r) * s + o[1]
    local z = sin(r) * s + o[3]
    return 150 * x / z + 64, 150 * y / z + 64, z
  end

  local function ag(g)
    local gg = {}
    local u, v, u1, v1 = (rnd(sn)), (rnd(sn))
    for c = 1, 8 do
...
function()
  for c = 1, #balls do
    local t = (time() / 4) + c / 3
    local b = balls[c]
    b.o = {sin(t * b.a[1]), cos(t * b.a[2]), 5 + sin(t * b.a[3])}
    for g in all(b.g) do
      if rnd() < 0.2 then
        print(#g)
        for d = #g, 2, -1 do
  ...
  for c = 1, #balls do
    ...
    for d = 1, #b.g do
      for e = 1, #b.g[d] do
        ...
        --line(x1+rnd(2), y1-rnd(), x2-rnd(2), y2+rnd(), gpal[gmode == 2 and flr(g[5]) or 1])
